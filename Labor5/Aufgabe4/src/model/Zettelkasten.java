...
/**
 * Die Klasse model.Zettelkasten beschäftigt sich mit der Verwaltung von Medien.
 * Es stehen Methoden zur Verfügung, um Medien in einer Liste hinzuzufügen,
 * zu entfernen und ein model.Medium nach seinem Titel zu finden.
...
public class Zettelkasten implements Iterable<Medium>, Serializable {
    ...
    /**
     * Gibt den model.Zettelkasten zurück.
     * @return Der model.Zettelkasten
     */
    ...
    /**
     * Setzt den model.Zettelkasten.
     * @param _myZettelKasten Der neue model.Zettelkasten
     */
     ...
     * @return true, wenn das übergebene model.Medium erfolgreich hinzugefügt wurde, anderweitig false
     */
    public boolean addMedium(Medium _medium){
        if (_medium == null) {
            System.out.println("Fehler: Es wurde kein model.Medium übergeben (null).");
            return false;
        }
    ...
    /**
     * Methode, um Medien vom model.Zettelkasten zu entfernen.
     * @param _title Titel vom model.Medium, welches gelöscht werden soll
     ...
     * @return true, wenn das model.Medium erfolgreich aus der Liste gelöscht wurde, anderweitig false
     */
    public boolean dropMedium(String _title, boolean _removeDuplicates, int _ID){
        ...
        if (gefundeneMedien.size() == 1){
            ...
            this.currentSortOrder = null;
            System.out.println("model.Medium mit der ID " + gefundeneMedien.get(0).getID() + " und dem Titel " + gefundeneMedien.get(0).getTitel() + " wurde erfolgreich entfernt.");
            return true;
        }
            ...
            if (_removeDuplicates){
                ...
                for (int i = 0; i < gefundeneMedien.size(); i++){
                    if (gefundeneMedien.get(i).getID() == _ID){
                        ...
                        this.currentSortOrder = null;
                        System.out.println("model.Medium mit der ID " + _ID + " und dem Titel " + _title + " wurde erfolgreich entfernt.");
                        return true;
                    }
                }
                System.out.println("Es wurde kein model.Medium mit dem Titel " + _title + " und der ID " + _ID + " gefunden.");
                return false;
            }
        }
    }
     ...
     * 1. Nach dem Titel (alphabetisch, basierend auf der compareTo Methode in model.Medium)
     * 2. Nach dem Medientyp (alphabetisch nach dem Klassennamen, z.B. "model.Buch" vor "model.CD")
    ...
    /**
     * Erstellt ein model.WikiBook Objekt, führt die Suche und das Parsing durch und behandelt Weiterleitungen iterativ.
     * Fügt das gefundene und geparste model.Medium anschließend dem model.Zettelkasten hinzu.
     * @param _titel Der zu suchende Titel des WikiBooks
     * @return true, wenn das model.Medium erfolgreich gefunden, geparst und hinzugefügt wurde, andernfalls false
     */
    public boolean addWikiBook(String _titel) {
        String currentTitle = _titel;
        System.out.println("Suche nach: " + _titel);
